...
function Collection(props) {
  ...
  const [activeTab, setActiveTab] = useState(IMAGE); //default image
  ...
  const fetchPosts = (option) => {
    ...
    const opt = {
      method: "GET",
      url: url, //we can also write as url,  , it is okay
      headers: {
  ...
  const showPost = (type) => {
    //wait 3 seconds and show all pictures
    //if not, give elasticSearch time to deal
    //save or fetch first 先保存还是获取
    setActiveTab(type);
    setTimeout(() => {
      setSearchOption({
        type: SEARCH_KEY.all,
